...
def train_mul(adata, adjl, adji, adjx, cosl, cosi, cosx, knn=10, h=[3000,3000], n_epochs=200,lr=0.0001, key_added='stMMR', random_seed=110,res=1,
          l=1.5,weight_decay=0.0001,a=10,b=1,c=10,embed=True,radius=0,enhancement=False,cluster="kmeans",loss_type="consistency",
                device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
    set_seed(random_seed)
    current_position = 0

    # adjl
    total_size = sum(adj.shape[0] for adj in adjl)
    block_diag_adjl = np.zeros((total_size, total_size))
    for adj in adjl:
        adj = np.exp(-1 * (adj ** 2) / (2 * (l ** 2)))
        adj = np.asarray(adj + sp.eye(adj.shape[0]))
        adj = normalize(adj)
        size = adj.shape[0]
        block_diag_adjl[current_position:current_position + size, current_position:current_position + size] = adj

    # adji
    total_size = sum(adj.shape[0] for adj in adji)
    block_diag_adji = np.zeros((total_size, total_size))
    for adj in adji:
        adj = np.exp(-1 * (adj ** 2) / (2 * (l ** 2)))
        adj = np.asarray(adj + sp.eye(adj.shape[0]))
        adj = normalize(adj)
        size = adj.shape[0]
        block_diag_adji[current_position:current_position + size, current_position:current_position + size] = adj

    # adjx
    total_size = sum(adj.shape[0] for adj in adjx)
    block_diag_adjx = np.zeros((total_size, total_size))
    for adj in adjx:
        adj = np.exp(-1 * (adj ** 2) / (2 * (l ** 2)))
        adj = np.asarray(adj + sp.eye(adj.shape[0]))
        adj = normalize(adj)
        size = adj.shape[0]
        block_diag_adjx[current_position:current_position + size, current_position:current_position + size] = adj
    ...
    combined_adata.obsm['adjl'] = block_diag_adjl
    combined_adata.obsm['adji'] = block_diag_adji
    combined_adata.obsm['adjx'] = block_diag_adjx
